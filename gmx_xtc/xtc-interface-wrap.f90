...
!   calculations. After the loops call close.
    ...
    interface 
      ...
      integer(C_INT) function read_xtc_numframes(filename,NFRAMES) bind(C, name='read_xtc_numframes')
        import
        character(kind=C_CHAR), intent(in) :: filename
...
contains
    ...
    subroutine read_xtc_count(xtc, filename_in, nframe)
        use, intrinsic :: iso_c_binding, only: C_NULL_CHAR, C_CHAR, c_f_pointer
        ...
        istat = read_xtc_numframes(filename,xtc % NFRAMES)
        if (istat == 0) nframe = xtc % NFRAMES
        write(*,*) 'xtc count frames : ',nframe, filename, xtc % NFRAMES
 
    end subroutine read_xtc_count
